...
| C3 | ~~**Role of 1197 Steve Biko Road**~~ | ✅ **RESOLVED 2026-07-27** — client instruction confirms 1197 is a second Vision Motors workshop (the engine shop), Branch Manager Jacques du Randt. Now modelled as two branches (site-config `branches`, `#engine-shop` schema branchOf, location/Contact/Booking). | ✅ Done |
...
| 2026-07-27 | **Second branch confirmed** — C3 resolved. 1197 Steve Biko Road (0084) is the engine-shop branch, Branch Manager Jacques du Randt (B4); 1059 main branch, Branch Manager Christo Vorster (B3, title HTML-sourced). Modelled as two locations (`branchOf`). |
| 2026-07-27 | **Legacy HTML audited** — see `LEGACY-HTML-CONTENT-AUDIT.md`. Client decisions: founding year "1992/34 yrs" **kept gated** (C4 — HTML bios internally impossible); legal name "Vision Value Service Centre t/a" **held** (C6); bodywork/insurance "approved by major insurers" **held**. |
| 2026-07-27 | **Re-upheld from the HTML:** "5-STAR RMI MEMBER" (E1/E11), RMI/MIWA/ARASA badges (C2), insurer-approval + named insurers (E8), manager bios/ages (E9), "VAT Registered" (unverified) — none published. |
| 2026-07-27 | **Diagnostic strip-and-assess policy** added (client-confirmed) — canonical `config/diagnostic-policy-config.ts`, rendered on homepage diagnostics, FAQ (+schema), booking form (note, no checkbox), booking page, About, Terms. No fixed price, no free-diagnostics/stripping, no guaranteed acceptance. |
| 2026-07-27 | Postcode: **0084 published for 1197 only** (confirmed); 1059 postcode still omitted (C20 — GBP says 0031). |
