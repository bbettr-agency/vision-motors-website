...
> - **The Engine Shop/Vision Motors** — 999 Steve Biko Rd, Wonderboom South, Pretoria, **0031** · Call/WhatsApp **069 007 2402** (`tel:+27690072402` / `wa.me/27690072402`) · Jacques du Randt.
...
>
> **🚨 Booking WhatsApp routing (booking + contact forms):** on submit the lead is POSTed to `/api/booking` FIRST (GHL/UTM/GCLID/validation/honeypot all preserved), then the selected workshop opens the correct branch WhatsApp (`wa.me/<num>?text=<url-encoded message>`) — Vision Motors→`27828235178`, Engine Shop→`27690072402`. Verified both routes, never crossed. Campaign data stays server-side. `lib/tracking.ts` gained `trackWhatsApp` + branch-aware `trackDirections`/`trackCall("branch")`.
>
